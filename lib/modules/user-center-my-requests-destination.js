...
import { fetchSearch, clearTips } from '../actions/routes/user-center-my-requests-destination'
import { changeDestination } from '../actions/routes/user-center-my-requests'

class UserCenterMyRequestsDestination extends Component {
  ...
  handleLocationPickerOkClick(loc, name) {
    const { dispatch } = this.props
    const { item } = this.props.app.routes.userCenterMyRequestsDestination
  ...
  render() {
    ...
    const { tips } = this.props.app.routes.userCenterMyRequestsDestination.search
    return (
...
export default connect(state => state)(UserCenterMyRequestsDestination)
