...
class UserCenterMyTravels extends Component {
  ...
  render() {
    const { travel_list, invitation_list, request_list } = this.props.app.routes.userCenterMyTravels
    ...
    const travels = travel_list.map((item, index) => (
      <form key={item.travel_id}>
        ...
        <div className='weui_btn_area'>
          ...
          <input type='submit' value='取消行程' className='weui_btn weui_btn_default' onClick={this.handleTravelCancelClick.bind(this, item)} />
        </div>
      </form>
    ))
      ...
      <DocumentTitle title='用户中心 - 我的行程'>
        <div style={styles.container} className='narrow'>
          ...
          <div style={{ flex: 1 }}>
            ...
            {/* getMyTravel */}
            { travels.length ? travels : <div className='weui_cells_title'>没有行程信息。</div> }
          </div>
        </div>
      </DocumentTitle>
    )
