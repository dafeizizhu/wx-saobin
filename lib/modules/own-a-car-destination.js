...
import { fetchSearch, clearTips } from '../actions/routes/own-a-car-destination'
import { changeDestination } from '../actions/routes/own-a-car'

class OwnACarDestination extends Component {
  ...
  handleLocationPickerOkClick(loc, name) {
    const { dispatch } = this.props
    const pickName = this.props.app.routes.ownACarDestination.search.loc.name
    if (pickName) {
      dispatch(changeDestination(loc, pickName))
    } else {
  ...
  render() {
    ...
    const { tips } = this.props.app.routes.ownACarDestination.search
    return (
...
export default connect(state => state)(OwnACarDestination)
