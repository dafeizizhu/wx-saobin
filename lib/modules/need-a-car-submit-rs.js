...
class NeedACarSubmitRs extends Component {
  ...
  render() {
    const { type, travel_list } = this.props.app.routes.needACarSubmitRs
    let content
    switch(type) {
      case 'search':
        const travelCells = travel_list.map(item => {
          return (
            <div key={item.id} style={styles.cell}>
              <UserInfo 
                keys={['avatar', 'nick_name', 'school', 'departure_time', 'departure_name', 'destination_name', 'passenger_num', 'vehicle_model', 'star', 'remark']}
                info={item} />
