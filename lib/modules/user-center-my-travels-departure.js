...
import { fetchSearch, clearTips } from '../actions/routes/user-center-my-travels-departure'
import { changeDeparture } from '../actions/routes/user-center-my-travels'

class UserCenterMyTravelsDeparture extends Component {
  ...
  handleLocationPickerOkClick(loc, name) {
    const { dispatch } = this.props
    const { item } = this.props.app.routes.userCenterMyTravelsDeparture
  ...
  render() {
    ...
    const { tips } = this.props.app.routes.userCenterMyTravelsDeparture.search
    return (
...
export default connect(state => state)(UserCenterMyTravelsDeparture)
