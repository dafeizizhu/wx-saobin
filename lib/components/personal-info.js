...
class PersonalInfo extends Component {
  ...
  render() {
    ...
    const driverInfo = [
      <InputCell label='驾照号码' key='license_no'><input className='weui_input' onChange={this.handleLicenseNoChange.bind(this)} defaultValue={license_no} required='required'/></InputCell>,
      <UploadCell label='上传驾照照片' key='license_pic'>
        ...
        <div className='weui_uploader_input_wrp'>
          <input className='weui_uploader_input' type='file' accept='image/*' onChange={this.handleLicensePicChange.bind(this)} required='required'/>
        </div>
      </UploadCell>,
      <Cell label='添加车辆信息' key='label_vehicle_info' />,
      <InputCell key='vehicle_1_brand' label='品牌'><input className='weui_input' onChange={this.handleVehicle1BrandChange.bind(this)} defaultValue={vehicle_1_brand}/></InputCell>,
      <InputCell key='vehicle_1_model' label='车型'><input className='weui_input' onChange={this.handleVehicle1ModelChange.bind(this)} defaultValue={vehicle_1_model}/></InputCell>,
      <InputCell key='vehicle_1_num' label='可载人数'><input className='weui_input' type='number' step='1' onChange={this.handleVehicle1NumChange.bind(this)} defaultValue={vehicle_1_num}/></InputCell>,
    ...
    return (
      <div className='weui_cells weui_cells_form'>
        <InputCell label='学校'><input className='weui_input' onChange={this.handleSchoolChange.bind(this)} defaultValue={school} required='required'/></InputCell>
        <InputCell label='姓名'><input className='weui_input' onChange={this.handleNameChange.bind(this)} defaultValue={name} required='required'/></InputCell>
        <InputCell label='昵称'><input className='weui_input' onChange={this.handleNickNameChange.bind(this)} defaultValue={nick_name} required='required'/></InputCell>
        <InputCell label='学校邮箱'><input className='weui_input' onChange={this.handleEmailChange.bind(this)} defaultValue={email} type='email' required='required'/></InputCell>
        { password ? [
          <InputCell key='pwd' label='密码'><input className='weui_input' onChange={this.handlePwdChange.bind(this)} type='password' required='required' /></InputCell>,
          <InputCell key='pwd2' label='确认密码'><input className='weui_input' onChange={this.handlePwd2Change.bind(this)} type='password' required='required' /></InputCell>
        ] : '' }
        <InputCell label='入学时间'><input className='weui_input' onChange={this.handleEnrollmentTChange.bind(this)} defaultValue={enrollment_t} type='date' required='required'/></InputCell>
        <SwitchCell label='是否愿意成为车主'><input className='weui_switch' type='checkbox' defaultChecked={is_driver} onChange={this.handleIsDriverChange.bind(this)} /></SwitchCell>
