...
class UserCenterNav extends Component {
  render() {
    ...
    const links = keys.map(key => {
      return <div key={key}><Link to={linkItems[key][1]} activeStyle={styles.activeLink}>{linkItems[key][0]}</Link></div>
    })
    return (
